...
```
/check-implementation-readiness --scope auth        # Flag format
/check-implementation-readiness --scope=auth        # Flag with equals
/check-implementation-readiness auth                # Positional (last arg if matches scope pattern)
```
...
NOW: LOAD the FULL @_bmad/bmm/workflows/3-solutioning/check-implementation-readiness/workflow.md, READ its entire contents and follow its directions exactly!
