...
```
/quick-dev --scope auth        # Flag format
/quick-dev --scope=auth        # Flag with equals
/quick-dev auth                # Positional (last arg if matches scope pattern)
```
...
NOW: LOAD the FULL @_bmad/bmm/workflows/bmad-quick-flow/quick-dev/workflow.md, READ its entire contents and follow its directions exactly!
