...
The API testing principles below are universal. Code examples use TypeScript/Playwright as reference implementations.

**Before generating code for other languages, fetch current patterns:**

```
Search: "[YOUR_LANGUAGE] API testing framework [CURRENT_YEAR]"
Search: "[YOUR_LANGUAGE] HTTP client testing best practices [CURRENT_YEAR]"
```

**Framework-specific API testing tools:**
| Language | HTTP Client | Test Framework | Schema Validation |
|----------|-------------|----------------|-------------------|
| JavaScript/TS | Playwright request, Axios, fetch | Jest, Vitest, Playwright | Zod, Joi, Yup |
| Python | requests, httpx, aiohttp | pytest, unittest | Pydantic, marshmallow |
| Java | RestAssured, OkHttp | JUnit, TestNG | JSON Schema Validator |
| Go | net/http, resty | testing, testify | go-playground/validator |
| C# | HttpClient, RestSharp | NUnit, xUnit | FluentValidation |
| Ruby | HTTParty, Faraday | RSpec, Minitest | dry-validation |
| PHP | Guzzle, Symfony HttpClient | PHPUnit, Pest | respect/validation |

**Universal API testing principles (ALL languages):**

- Test APIs directly without browser overhead when validating service logic
- Use schema validation to verify response structure
- Test error handling (4xx, 5xx) explicitly
