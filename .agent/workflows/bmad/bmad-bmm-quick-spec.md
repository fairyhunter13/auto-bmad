...
```
/quick-spec --scope auth        # Flag format
/quick-spec --scope=auth        # Flag with equals
/quick-spec auth                # Positional (last arg if matches scope pattern)
```
...
NOW: LOAD the FULL @_bmad/bmm/workflows/bmad-quick-flow/quick-spec/workflow.md, READ its entire contents and follow its directions exactly!
