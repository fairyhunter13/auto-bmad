...
```
/workflow --scope auth        # Flag format
/workflow --scope=auth        # Flag with equals
/workflow auth                # Positional (last arg if matches scope pattern)
```
...
NOW: LOAD the FULL @_bmad/bmb/workflows/workflow/workflow.md, READ its entire contents and follow its directions exactly!
